...
from RunModules.run_oscillatory_machine import run_oscillatory_machine_wth_floating_gate as run_machine
    ...
    def compute_reservoir_states(self,
        ...
        for idx in tqdm(range(ind_start, ind_end), desc="Creating signals for offline training"):
            ...
            if save:
                ...
                # for k in range(len(H_list)-3, len(H_list)):
                for k in range(0, len(H_list), 50):
                    # # A = self.net.Adj
                    A = H_list[k] #self.net.Condmat
                    g = connected_components(nx.from_scipy_sparse_array(A, parallel_edges=False, create_using=None))[0]
                    eig = nx.laplacian_spectrum(g) #, weight=)
                    np.save(arr=eig, file=os.path.join(save_temp, 'eigenvalues_{:06d}.npy').format(k))

                    ## matL = csr_matrix(csgraph.laplacian(H_list[k], normed=False)).todense()
                    ## eig, _ = np.linalg.eigh(matL)

                    # tau_range = np.logspace(-2, 6, num=1000)
                    # S = von_neumann_entropy_numpy(tau_range=tau_range, lambd=eig)[0] / np.log(len(eig))
                    # C = np.log(len(eig)) * specific_heat(tau_range=tau_range, spec_ent=S, batch_dim=0)
                    # plt.semilogx(tau_range, S, label='S')
                    # plt.semilogx(tau_range, C, label='C')
                    # plt.xlabel(r'$\tau$')
                    # plt.legend()
                    # plt.show()
