    ...
    def plot_network(G,
        ...
        if len(node_color) == 0:
            ...
            nx.draw_networkx_edges(G, pos=dict(coordinates), alpha=edge_alpha, width=edge_width, edge_color=weights,
                                   edge_cmap=edge_cmap, edge_vmin=edge_vmin, edge_vmax=edge_vmax, ax=ax)

            # Draw nodes with node_alpha
            nx.draw_networkx_nodes(G, pos=dict(coordinates), node_color=node_color, node_size=node_size,
                                   cmap=nodes_cmap, vmin=vmin, vmax=vmax, alpha=node_alpha, node_shape=node_shape,
                                   ax=ax)

            # Colorbars for nodes and edges
            shrink_bar = 0.65
            pad = .001
            ticks_num = 2
            if cb_nodes_lab != '':
