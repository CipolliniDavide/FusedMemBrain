...
def unroll_list(input_list):
    result = []
    for item in input_list:
        if isinstance(item, list):
            result.extend(unroll_list(item))
        else:
            result.append(item)
    return result
...
class utils:
    ...
    def save_dict_to_txt(dict, name, savepath):
        # open file for writing
        f = open("{:s}{:s}.txt".format(savepath, name), "w")
        # write file
        f.write(str(dict))
        # close file
        f.close()

    def load_dict_from_txt(path, name):
        import ast  # Import ast for safe evaluation

        with open(path + name + '.txt', 'r') as file:
            dict_data = ast.literal_eval(file.read().strip())  # Safely evaluate the entire dictionary
        return dict_data
