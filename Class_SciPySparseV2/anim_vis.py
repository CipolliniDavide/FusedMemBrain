...
def plot_H_evolution(H_Horizon,
                     node_labels,
                     t_list_Horizon,
                     signals_1rst_row=None,  # Set default to None
                     coordinates=None,
                     node_voltage_list=None,
                     src=None,
                     signals_2nd_row=None,  # Set default to None
                     save_path='',
                     ...
                     background_color='lightgrey'  # New parameter for background color
                     ):
    ...
    reversed_cmap = mcolors.ListedColormap(cmap(np.linspace(1, 0, cmap.N)))

    for time_index in tqdm(np.linspace(0, len(H_Horizon), endpoint=False, num=number_of_plots, dtype=int),
        ...
        # Create figure and axes based on the presence of signal rows
        if signals_1rst_row is None and signals_2nd_row is None:
            # No signal rows, just plot the network
            fig, ax = plt.subplots(figsize=figsize)

        else:
            # Create a gridspec for plotting with subplots for signals
            fig, ax = plt.subplots(figsize=figsize)

            ax.tick_params(bottom=False, labelbottom=False, left=False, labelleft=False)
            axgrid = fig.add_gridspec(8, 4)
            ax = fig.add_subplot(axgrid[0:6, :])  # Main network plot

        # Set the background color of the network plot
        ax.set_facecolor(background_color)

        # Plot the network graph
        ax = visualize.plot_network(ax=ax, G=H, figsize=(8, 6),
                                    labels=node_labels if show_name_labels else None,
                                    node_color=list(node_voltage),
                                    ...
                                    vmin=np.min(node_voltage),
                                    vmax=np.max(node_voltage),
                                    edge_vmin=edge_min-(edge_max-edge_min)*.1,
                                    edge_vmax=edge_max,
                                    nodes_cmap=reversed_cmap,
                                    edge_cmap=edges_cmap,
                                    weight_key='weight')

        # Find and circle specific nodes
        red_nodes = [n[0] for n in node_labels if n[1].startswith('Inp')]
        green_nodes = [n[0] for n in node_labels if n[1].startswith('n')]

        nx.draw_networkx_nodes(H, pos=dict(coordinates), nodelist=red_nodes,
                               node_size=node_size * 3,
                               edgecolors=inpnode_c, linewidths=5, node_color='none', ax=ax,
                               node_shape=inputnode_shape)

        nx.draw_networkx_nodes(H, pos=dict(coordinates), nodelist=green_nodes, node_size=node_size * 2.5,
                               edgecolors=activenode_c, linewidths=5, node_color='none', ax=ax,
                               node_shape=activenode_shape)  # Square shape for "n" nodes

        # Plot the first row of signals if available
        if signals_1rst_row is not None and len(signals_1rst_row) > 0:
            ax1 = fig.add_subplot(axgrid[6:7, :])
            ax1 = plot_voltage_input_with_time_bar(ax=ax1, V_list=signals_1rst_row, t_list=t_list_Horizon,
                                                   time_index=time_index, src=src,
                                                   ylabel=y_label_1rst_row, xlabel=None)
            ax1.set_xlim(left=t_list_Horizon.min(), right=t_list_Horizon.max())

        # Plot the second row of signals if available
        if signals_2nd_row is not None and len(signals_2nd_row) > 0:
            ax2 = fig.add_subplot(axgrid[7:, :])
            ax2 = plot_voltage_input_with_time_bar(ax=ax2, V_list=signals_2nd_row, t_list=t_list_Horizon,
                                                   time_index=time_index, src=src,
                                                   ylabel=y_label_2ndrow, xlabel='Time [s]')
            ax2.set_xlim(left=t_list_Horizon.min(), right=t_list_Horizon.max())

            # Optional: Add highlighting if `t1` is provided
            if t1:
        ...
        # Set the figure background color to ensure it doesn't override the axis color
        fig.patch.set_facecolor(background_color)  # Set the figure's background color
        # fig.patch.set_alpha(0.6)

        plt.suptitle(title, weight='bold', size='xx-large')
        plt.tight_layout()
        plt.savefig(save_path + '{:04d}.{:s}'.format(time_index, image_format), dpi=300)
        plt.close('all')
...
def plot_voltage_input_with_time_bar(ax, V_list, t_list, src, time_index, ylabel='Volt. [V]', xlabel='Time [s]',
    ...
    if xlabel:
        ...
        x_ticks = np.linspace(np.min(t_list), np.max(t_list), 5)
        x_ticks_label = ['{:.1f}'.format(t) for t in x_ticks]
    ...
    ax.axvline(x=t_list[time_index], color='r', linewidth=4)
...
#
#         ax = visualize.plot_network(ax=ax, G=H, figsize=figsize, labels=node_labels,
