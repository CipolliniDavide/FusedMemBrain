...
net_param = edict({'rows': 1,  # 31,
                   'cols': 1,  # 31,
                   'frac_of_static_elements': 0,  # fraction of resistors in the network
                   'weight_init': 'rand',  # rand, if rand start with random, else: None
...
if __name__ == "__main__":
    ...
    src = [
        node1]  # ,30] #[node_map(3, 1, rows, cols), 10] # define a list of source nodes in the range [0, rows*cols-1]
    ...
    # Instantiate Classes
    H_list, curr_rec = net.run(t_list=time_array, sourcenode_list=src, groundnode_list=gnd, V_list=V_list)
